#include "sbox.h"
#define GIT_BIT(VAR,BIT_NUM) ((VAR & (1<<BIT_NUM) )>>BIT_NUM)
 ...
 /* function takes the expanded 48 bits and convert them to 32 bits using sbox tables*/     
uint32_t sbox(uint64_t sbox_input)
{
    uint64_t mask = 0x0000fc0000000000;
    uint64_t s_group_value;
    uint8_t row_num;
    uint8_t col_num;
    uint32_t sbox_out =0;
    for (int i = 0;i < NUM_OF_BLOCKS;i++)
    {
        s_group_value = mask & sbox_input;
        s_group_value = s_group_value >>(SBOX_INPUT_SIZE - (i+1) *SIZE_OF_S_BLOCK_INPUT);
        row_num = (s_group_value & 0x01) | (GIT_BIT(s_group_value , 5) <<1);
        col_num = (s_group_value >>1) & 0x0f;
        sbox_out |= s[i][row_num][col_num] << (SBOX_OUTPUT_SIZE - (i+1)*SIZE_OF_S_BLOCK_OUTPUT);
        mask>>=SIZE_OF_S_BLOCK_INPUT;
    }
    return sbox_out;
}